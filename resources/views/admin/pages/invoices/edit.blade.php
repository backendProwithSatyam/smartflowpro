                    ...
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <div class="mt-3">
                                ...
                                <input type="text" name="invoice_subject" class="form-control" 
                                       value="{{ old('invoice_subject', $invoice->invoice_subject) }}" required>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="d-flex align-items-center">
                                            <span class="me-2">#{{ $invoice->invoice_number }}</span>
                                        </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="editable-field" onclick="editField(this, 'issued_date')">
                                            <span id="issued_date_display">{{ $invoice->issued_date ? $invoice->issued_date->format('M j, Y') : 'Date Sent' }}</span>
                                            <input type="date" name="issued_date" class="form-control d-none" 
                                                   value="{{ old('issued_date', $invoice->issued_date ? $invoice->issued_date->format('Y-m-d') : '') }}">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="editable-field" onclick="editPaymentDue(this)">
                                            <span id="payment_due_display">{{ ucfirst(str_replace('_', ' ', $invoice->payment_due)) }}</span>
                                            <select name="payment_due" class="form-control d-none" onchange="updatePaymentDue()">
                                                <option value="upon_receipt" {{ $invoice->payment_due == 'upon_receipt' ? 'selected' : '' }}>Upon Receipt</option>
                                                <option value="net_15" {{ $invoice->payment_due == 'net_15' ? 'selected' : '' }}>Net 15</option>
                                                <option value="net_30" {{ $invoice->payment_due == 'net_30' ? 'selected' : '' }}>Net 30</option>
                                                <option value="net_45" {{ $invoice->payment_due == 'net_45' ? 'selected' : '' }}>Net 45</option>
                                                <option value="custom" {{ $invoice->payment_due == 'custom' ? 'selected' : '' }}>Custom</option>
                                            </select>
                                        </div>
                                        <div id="custom_due_date" class="mt-2 {{ $invoice->payment_due == 'custom' ? '' : 'd-none' }}">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="salesperson" class="form-control">
                                            ...
                                            @foreach($users as $user)
                                                <option value="{{ $user->id }}" {{ $invoice->salesperson == $user->id ? 'selected' : '' }}>
                                                    {{ $user->name }}
                                                </option>
                                            @endforeach
                                        </select>
                ...
                <div class="card-body">
                    <div class="summary-section">
                        ...
                        <div class="summary-row">
                            ...
                            <span id="discount">${{ number_format($invoice->discount_amount, 2) }}</span>
                        </div>
                        <div class="summary-row">
            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <textarea name="client_message" class="form-control" rows="4" 
                              placeholder="Enter message for client...">{{ old('client_message', $invoice->client_message) }}</textarea>
                </div>
            </div>
            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <textarea name="contract_disclaimer" class="form-control" rows="3" 
                              placeholder="Enter contract terms and disclaimer...">{{ old('contract_disclaimer', $invoice->contract_disclaimer) }}</textarea>
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="internal_notes" class="form-control" rows="4" 
                                  placeholder="Enter internal notes...">{{ old('internal_notes', $invoice->internal_notes) }}</textarea>
                    </div>
...
<script>
    ...
    function editField(element, fieldName) {
        ...
        input.addEventListener('blur', function() {
            span.textContent = input.value ? new Date(input.value).toLocaleDateString() : 'Date Sent';
    ...
    // Line items
    function addLineItem(type, existingData = null) {
        lineItemCount++;
            ...
            <div class="line-item" data-type="${type}">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <textarea name="line_items[${lineItemCount}][description]" class="form-control" rows="2">${existingData ? existingData.description : ''}</textarea>
                    </div>
                </div>
                <div class="row mt-2">
                    <div class="col-md-2">
                        ...
                        <input type="number" name="line_items[${lineItemCount}][quantity]" class="form-control quantity" 
                               value="${existingData ? existingData.quantity : 1}" min="0" step="0.01" onchange="calculateLineTotal(this)" required>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Unit Price</label>
                        <input type="number" name="line_items[${lineItemCount}][unit_price]" class="form-control unit-price" 
                               value="${existingData ? existingData.unit_price : 0}" min="0" step="0.01" onchange="calculateLineTotal(this)" required>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Total</label>
                        <input type="number" name="line_items[${lineItemCount}][total]" class="form-control total" 
                               value="${existingData ? existingData.total : 0}" min="0" step="0.01" readonly>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Service Date</label>
                        <div class="editable-field" onclick="editServiceDate(this, ${lineItemCount})">
                            <span id="service_date_${lineItemCount}_display">${existingData && existingData.service_date ? new Date(existingData.service_date).toLocaleDateString() : 'Set Service Date'}</span>
                            <input type="date" name="line_items[${lineItemCount}][service_date]" class="form-control d-none" 
                                   value="${existingData ? existingData.service_date : ''}">
                        </div>
                    </div>
                    <div class="col-md-3">
