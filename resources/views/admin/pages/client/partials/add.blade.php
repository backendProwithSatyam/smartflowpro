    ...
    <div class="d-flex justify-content-center align-items-center" style="min-height: 70vh;">
        <div class="container">
            <div class="form-container">
                <h2 class="mb-4 fw-bold text-dark">New Client</h2>
                <form id="clientForm" method="POST"
                    ...
                    @endif
                    <div class="mb-4">
                        <h5 class="fw-semibold text-dark mb-2">Primary contact details</h5>
                        <p class="text-muted-custom mb-4">Provide the main point of contact to ensure smooth communication
                            and
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                ...
                                <select class="form-select" id="title" name="title">
                                    ...
                                    <option value="Mrs" {{ $editMode && $client->title === 'Mrs' ? 'selected' : '' }}>Mrs
                                    </option>
                                    <option value="Miss" {{ $editMode && $client->title === 'Miss' ? 'selected' : '' }}>Miss
                                    </option>
                                    <option value="Ms" {{ $editMode && $client->title === 'Ms' ? 'selected' : '' }}>Ms
                                    </option>
                                    <option value="Dr" {{ $editMode && $client->title === 'Dr' ? 'selected' : '' }}>Dr
                                    </option>
                                    <option value="Prof" {{ $editMode && $client->title === 'Prof' ? 'selected' : '' }}>Prof
                                    </option>
                                </select>
                    ...
                    <div class="section-divider">
                        <h5 class="fw-semibold text-dark mb-3">Lead information</h5>
                        <div class="row g-3">
                            <div class="col-md-6">
                    ...
                    <div class="section-divider">
                        <h5 class="fw-semibold text-dark mb-3">Property Address Details</h5>
                        ...
                        <button type="button" id="addAddressBtn" class="btn btn-outline-primary mt-2">
                    ...
                    <div id="billingAddressSection"
                        ...
                        @include('admin.pages.client.partials.address', ['index' => 'billing', 'address' => $client->addresses->where('type', 'billing')->first() ?? [], 'taxRates' => $taxRates, 'billing' => true])
                    </div>
                    <div class="row mt-4">
                        <div class="form-actions-fixed">
                            <button type="submit"
                                class="btn btn-primary me-2">{{ $editMode ? 'Update Client' : 'Create Client' }}</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            addressContainer.addEventListener('click', function (e) {
                if (e.target.classList.contains('remove-address-btn') || e.target.closest('.remove-address-btn')) {
            ...
            function updateAddressLabels() {
                ...
                addressGroups.forEach((group, index) => {
                    ...
                    inputs.forEach(input => {
                        ...
                        if (name) {
                            const updatedName = name.replace(/addresses\[\d+\]/, `addresses[${index}]`);
                ...
                if (sameAsBilling) {
                    ...
                    if (firstAddress) {
                        ...
                        fields.forEach(field => {
                            const propInput = firstAddress.querySelector(`input[name="addresses[0][${field}]"], select[name="addresses[0][${field}]"]`);
                            const billInput = document.querySelector(`input[name="billing_address[${field}]"], select[name="billing_address[${field}]"]`);
                            if (propInput && billInput) {
                                billInput.value = propInput.value;
                            }
                        });
                        // For tax_rate_id
                        const propTax = firstAddress.querySelector('select[name="addresses[0][tax_rate_id]"]');
                        const billTax = document.querySelector('select[name="billing_address[tax_rate_id]"]');
                        if (propTax && billTax) {
